...
def get_foldwise_split(db_idx, fold_no, number_of_folds, imgs_masks_pairs, read_from_file=False, save_split_to_file=True, with_test_set=False):
    if read_from_file:
        with open(f'db_{db_idx}_fold_{fold_no}_out_of_{number_of_folds}_debug_files_list.json', 'r') as f:
            split = json.load(f)
            current_train_set = [pair for pair in imgs_masks_pairs if [os.path.basename(pair[0]), os.path.basename(pair[1])] in split['training']]
            current_val_set = [pair for pair in imgs_masks_pairs if [os.path.basename(pair[0]), os.path.basename(pair[1])] in split['validation']]
            test_set = [pair for pair in imgs_masks_pairs if [os.path.basename(pair[0]), os.path.basename(pair[1])] in split['test']]
    else:
        if with_test_set:
            rest, test_set = train_test_split(imgs_masks_pairs, test_size=0.2, random_state=12345)
        else:
            rest = imgs_masks_pairs
            test_set = []
        ...
        if save_split_to_file:
            ...
            with open(f'db_{db_idx}_fold_{fold_no}_out_of_{number_of_folds}_debug_files_list.json', 'w') as f:
...
def get_experiment_model_name(experiment_name, fold_no):
    return f'{experiment_name}_{fold_no}.keras'
...
def load_files(dataset_dir, test_set=None, img_file_pattern="*.jpg"):
    imgs_files_paths = list(sorted(glob.glob(os.path.join(dataset_dir, img_file_pattern))))
    if test_set is not None:
        imgs_files_paths = [file_path
                            for file_path
                            in imgs_files_paths
                            if os.path.basename(file_path) in test_set]
